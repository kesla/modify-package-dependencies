...
const inject = getPackage => {
  ...
  return {
      ...
      return getNameAndVersion(arg)
        .then(({name, version}) => {
          ...
          if (packageJson.devDependencies) {
            result = set(packageJson, 'devDependencies', without(packageJson.devDependencies, name));
          }
    ...
    addDev: (packageJson, arg) => {
      ...
      return getNameAndVersion(arg)
        .then(({name, version}) => setIn(packageJson, ['devDependencies', name], version));
    },
    remove: (packageJson, packageName) => {
      let result = packageJson;

      if (packageJson.dependencies) {
        result = set(result, 'dependencies', without(packageJson.dependencies, packageName));
      }

      if (packageJson.devDependencies) {
        result = set(result, 'devDependencies', without(packageJson.devDependencies, packageName));
      }
